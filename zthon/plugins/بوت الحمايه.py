...
حقـوق : @xsara_v
@xsara_v - كتـابـة الملـف :  زلــزال الهيبــه"""
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس ساره",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت ساره",
...
async def check_edit_media(event):
    if not event.is_group:
        return
    if event.is_group: #Write Code By T.me/xsara_v
        chat = await event.get_chat()
        admin = chat.admin_rights
        creator = chat.creator
        if not admin and not creator:
            return
    zed_dev = (925972505, 1895219306, 5280339206)  #Write Code By T.me/xsara_v
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
    user = await event.get_sender()
    if is_locked(zed_id, "document") and event.message.media: #Write Code By T.me/xsara_v
        if zelzal == malath or zelzal in zed_dev:
            return
        else:
	        try:
	            await event.delete() #Write Code By T.me/xsara_v
	        ...
	        except Exception:  #Write Code By T.me/xsara_v
	            update_lock(zed_id, "document", False)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝑆𝞝𝐴𝙏𝙃𝙊𝙉\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝑆𝞝𝐴𝙏𝙃𝙊𝙉\
