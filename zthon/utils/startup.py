    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                ...
                caption="**•⎆┊تـم بـدء تشغـيل سـورس ساره الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝑆𝙀𝐴𝙏𝙝𝙤𝙣𓅛", "https://t.me/xsara_v"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت ساره المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
